...
/*
The dp[i][j] saves how much more scores that the first-in-action player (Current Play, even for the 2nd, when he chooses) will get from i to j than the second player. First-in-action means whomever moves first. You can still make the code even shorter but I think it looks clean in this way.

这是一个区间dp的问题。
设dp[i][j]表示区间[i,j]里玩家1与玩家2的分数差值的最大值（玩家1-玩家2）。
转移方程为：dp[i][j] = max(num[i]-dp[i+1][j],num[j]-dp[i][j-1])dp[i][j]=max(num[i]−dp[i+1][j],num[j]−dp[i][j−1])
minmax
O(n^2)
 */
...
public class PredictWin {
    ...
    public boolean PredictTheWinnerOnSpace(int[] nums) {
        if (nums == null) {
            return true;
        }
        int n = nums.length;
